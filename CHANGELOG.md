...
Patch release. Fixes a runtime crash on first use of the v1.6.0 publish hooks.

### Fixed

- **`promise already under evaluation: recursive default argument reference`**
  error when calling `build_filtervariant_ro_crate()` or
  `build_phase_cog_ro_crate()`. The bug was a name collision in v1.6.0:
  the new optional parameter `crop_spec` shadowed the helper function of
  the same name in its own default expression
  (`crop_spec = crop_spec(plant)`), so R's lazy evaluator entered a
  recursive promise when the default fired. The parameter is renamed to
  `crop` across both publish hooks and the two helper functions in
  `_crop_specs.R` (`build_keywords()`, `build_subject_entities()`). The
  helper function `crop_spec()` keeps its name and public API.
- Affected only v1.6.0; v1.5.0 and earlier did not have the parameter.

### Migration

If any call site explicitly passes the optional parameter by name as
`crop_spec = ...`, change it to `crop = ...`. The default — looking the
crop up by `plant` — does not require any change at the call site.

```r
# Before (v1.6.0, crashed when crop_spec default fired):
build_filtervariant_ro_crate(..., crop_spec = my_custom_crop)

# After (v1.6.1+):
build_filtervariant_ro_crate(..., crop = my_custom_crop)
```
...
Multi-crop blueprint: the publish hooks are now parametric on a single
shared crop specification, so a per-crop deposit family for DWD plant
IDs 202, 203, 204, 205, 207, 208, and 215 can be regenerated from one
codebase. Domain semantics added via AGROVOC subject terms; quality and
provenance metadata unchanged. No numerical changes to any rasters, CSVs,
or model fits.
...
- **`_crop_specs.R`** new shared helper file in `function/`, sourced
  ...
  - DefinedTerm / DefinedTermSet entity builders that wrap AGROVOC URIs
    as proper JSON-LD nodes (`build_subject_entities()`)
...
- **`crop_spec` and `creators` parameters** on both
  ...
  Both default sensibly from `plant`, so existing call sites need no
  changes. Override at the call site for non-standard contributor lists
  or crops not yet in `_crop_specs.R`.
- **AGROVOC subject terms** on the root dataset of every crate via
  ...
  spatial interpolation on the PHASE side) and one shared DefinedTermSet
  entity describing AGROVOC itself, with `skos:exactMatch` cross-links
  to Wikidata. Resolves the Layer 6 "domain semantics" gap in the
  metadata stack (RO-Crate / DCAT / DCT / DQV / PROV-O / SKOS / ISO
  19157 / Schema.org / SPDX).
- **`agrovoc` namespace** in the `@context` of every crate
  (`http://aims.fao.org/aos/agrovoc/`), alongside the existing W3C and
  ISO namespaces.
- **Multi-creator support** with explicit DataCite-style role literals
  (`Producer`, `DataManager`). Each creator emits one Person entity in
  the `@graph` with `schema:roleName`, `prov:role`, and affiliation.
- **Layered keyword scheme** applied uniformly across both crate kinds:
  a mandatory family-wide core (`phenology`, `Germany`, `DWD`,
  `PhenoPhaseR`, `FAIR`, `RO-Crate`, `1 km resolution`, `1993–2024`),
  per-crop additions (`<common name>`, `<binomial>`, `DWD Plant ID
  <id>`), and artefact-specific additions (`growing degree days`,
  `PHASE model`, … for filter variants; `spatial interpolation`,
  `Cloud Optimized GeoTIFF`, `EPSG:25832`, … for PHASE). Replaces the
  previous hard-coded `"winter wheat"` literals in both builders.
- **Per-crop output directory** in `PhenoPhaseR.R`: `output_dir` is now
  derived from `plant` (`~/PhenoPhaseR/output/<plant>/`), so the
  commented crop switch at the top of the file is now genuinely a
  single-line reconfiguration of the whole run.
- **Crop-parametric run summary** in `PhenoPhaseR.R` using
  `crop_spec(plant)` for the human-readable crop name + binomial in
  the closing message.
...
- **JKI organisation entity** unified to a single canonical
  ...
  Plants` with ROR `022d5qt08`. Resolves the affiliation-string drift
  between the existing winter-wheat deposits at concept DOIs
  `19483111`, `19571847`, and `18743008`, where three different label
  forms had crept in.
- **JKI ROR** consolidated to `022d5qt08` (was `02jx3x895` in the
  builders, while the published deposits used `022d5qt08`). One ROR,
  one label, applied everywhere.
...
No API change. Existing `build_filtervariant_ro_crate()` and
`build_phase_cog_ro_crate()` call sites continue to work unchanged —
`crop_spec` and `creators` default from `plant` and the family policy
in `_crop_specs.R`. Drop `_crop_specs.R` into `function/` alongside
the two builders; both auto-source it at load time. Existing deposits
at the four concept DOIs are unaffected until republished against
v1.6.0; the change shape on republish is:

- root dataset gains `schema:about` + `dct:subject` arrays of AGROVOC
  `@id` references
- `@graph` gains one DefinedTerm entity per AGROVOC concept and one
  shared DefinedTermSet entity
- `@context` gains the `agrovoc` namespace
- Person entities gain `schema:roleName` + `prov:role` properties
- root dataset's `creator` / `dct:creator` arrays carry one entry per
  contributor instead of one
- JKI organisation entity carries the canonical name + ROR

Downstream code that walked `creator` arrays assuming a single creator
should now iterate. Downstream code that resolved authors by `@id`
needs no change.

### Why this change

The four winter-wheat deposits (input data, software, filter variants,
PHASE COGs) form a four-node provenance graph that we now want to
replicate for six additional crops. Doing that by hand would propagate
the keyword/affiliation/role inconsistencies already present in the
winter-wheat set. Centralising the per-crop and family-wide
configuration in `_crop_specs.R` makes the family genuinely a blueprint:
one source of truth, one set of defaults, one place to fix things.
AGROVOC adds the only major vocabulary that was missing from the stack
— domain subject semantics — and does so via the same SKOS-bridge
pattern already used for ISO 19157-1 in DQV. Together these changes
let the family read as a family in OpenAIRE, BonaRes, FAIRagro, and
any downstream catalogue that consumes Schema.org `about` or DCAT
`dct:subject`.
