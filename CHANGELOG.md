...
Patch release: completes the eight-crop family by adding sugar beet. No API or
behavioural change — the workflow already processed any crop present in
`.CROP_SPECS`; this adds the missing table entry and its verified vocabulary
anchor.

### Added

- **Sugar beet (DWD Plant ID 253) added to `.CROP_SPECS`.** Root crop
  (*Beta vulgaris*, German "Zucker-Ruebe"), modelled phases 10 (sowing),
  12 (emergence), 13 (canopy closure), 24 (harvest). Anchored to AGROVOC
  `sugar beet` (`c_7499`), maintainer-verified against agrovoc.fao.org and
  confirmed at build time by `verify_agrovoc_uris()`. With this the family
  covers all eight target crops: winter wheat (202), winter rye (203),
  winter barley (204), winter rapeseed (205), spring barley (207), oats
  (208), maize (215), sugar beet (253).

### Notes

The sugar beet Wikidata QID (`Q23800`) is carried but flagged unverified, like
all other crops' Wikidata identifiers; only the AGROVOC anchor is build-checked.

---

## [1.7.1] – 2026-06-14

Patch release: deposit keyword corrections. No code, API, or numerical
changes; only the free-text `keywords` / `dcat:keyword` emitted on the root
dataset of future deposits change.

### Changed

- **Keywords refreshed and made artefact-honest.** `build_keywords()` now
  distinguishes the two deposit kinds more accurately. The PHASE deposit gains
  validated-uncertainty terms (`uncertainty quantification`, `predictive
  uncertainty`, `prediction interval`, `cross-validation`, `calibration`) and
  method terms (`generalized additive model`, `GAM`, `COG`, `spatial data`),
  matching the k-fold-calibrated uncertainty layer shipped in 1.7.0. The
  filter-variant deposit gains intermediate-product terms (`station
  filtering`, `outlier detection`, `data curation`) but deliberately carries
  **no** uncertainty/COG terms it does not contain.

### Fixed

- **Removed the inaccurate `BBCH` keyword** from PHASE deposits. DWD
  phenological phase codes are crop-specific catalogue codes, not BBCH
  numbers, so the keyword was misleading for anyone filtering on it.
- **Temporal keyword now matches the deposit.** The hard-coded `1993–2024`
  keyword is replaced by a range derived from the actual covered years
  (`min(years)`–`max(years)`), passed by both builders, so it no longer drifts
  from the deposit's real coverage.
...
Reinstates the AGROVOC subject layer with **verified** URIs and a build-time
guard, makes all DQV quality measures anchor honestly to ISO 19157, and adds
a validated calibration of the BSE uncertainty layer. No numerical changes to
any raster or to the existing VAM/CAL/GEM tables; the changes are to metadata
encoding plus one new validation artefact (PIC).

### Added

- **AGROVOC subject layer reinstated with verified URIs.** Every per-crop and
  family-wide AGROVOC URI in `_crop_specs.R` was resolved against
  `agrovoc.fao.org` and corrected. Of the v1.6.0 set, only winter wheat
  (`c_8412`) and maize (`c_12332`) had been right; the other seven were
  wrong (e.g. phenology was `c_28793` → should be `c_5774`; growing degree
  days `c_36099` → `c_28c4c002`; Germany `c_3258` → `c_3245`). Winter rye and
  winter rapeseed have no winter-specific AGROVOC concept, so they anchor to
  the generic species concept (`agrovoc_is_generic = TRUE`) with the
  winter/spring distinction carried by the DWD Plant ID and the keyword.
  Spatial interpolation has no AGROVOC concept at all and is kept as a
  free-text keyword only. The PHASE deposits additionally anchor
  `spatial data` (`c_379bbe9f`, family) and `statistical uncertainty`
  (`c_28975`, for the BSE layer).
- **Build-time AGROVOC verification (`verify_agrovoc_uris()`).** Resolves
  every configured URI against the live AGROVOC REST API and reports any whose
  returned prefLabel no longer matches the concept the config claims. This is
  the guard that would have caught the v1.6.0 mis-typing. It distinguishes
  rate-limit (429, retried with backoff — never treated as a wrong URI),
  not-found (404 — blocks like a mismatch), and network errors (inconclusive).
  Throttled and curl-aware. `PhenoPhaseR.R` calls it report-only before the
  publish hooks; set `stop_on_mismatch = TRUE` for CI/publishing.
- **BSE prediction-interval calibration (PIC), k-fold cross-validated.**
  `spatial_interpolation.R` computes PICP (prediction-interval coverage
  probability) and MPIW (mean interval width) at nominal 90 % via a dedicated
  **k-fold cross-validation** pass (default 5 folds), separate from the
  production fit. The published DOY/BSE rasters are built from all stations;
  the calibration holds each station out exactly once across the folds, refits
  the BAM on the rest, and predicts at the held-out stations with `se.fit`,
  pooling the out-of-fold results. The interval combines each fold's `se.fit`
  with the production all-data residual variance — using the SE alone would
  understate coverage and mislabel a confidence interval as a prediction
  interval. Written to `PIC_<plant>-<phase>_<year>.csv`. Controlled by new
  `calibrate` (default TRUE for BAM, independent of `validation`/`uncertainty`)
  and `n_folds` (default 5) arguments. `.compute_pic_kfold()` is a standalone,
  unit-testable helper. `build_phase_cog_ro_crate.R` surfaces `picp`/`mpiw` as
  DQV `QualityMeasurement`s under `DQ_UsabilityElement`, each tied to the BSE
  COG via `dqv:computedOn`.

### Changed

- **Honest DQV measure anchoring (both builders).** Quality measures no
  longer mint `iso19157:<measure>` IRIs (`iso19157:cv_rmse`, etc.) — no such
  concept exists in ISO 19157, so those IRIs resolve to nothing.
  `schema:propertyID` now carries the bare measure token, and the fabricated
  `skos:closeMatch` to `iso19157:<measure>` is removed. The link to the
  standard is made only at the genuine dimension-class level via
  `dqv:inDimension` (`iso19157:DQ_ThematicAccuracy`, `DQ_UsabilityElement`,
  `DQ_CompletenessOmission`, `DQ_LogicalConsistency` — all real classes).
  Applied uniformly to every measure in both `build_phase_cog_ro_crate.R` and
  `build_filtervariant_ro_crate.R`, so the encoding stays coherent while being
  honest. This adopts the "own the measure, anchor the dimension" posture
  without committing to any unregistered namespace IRI.

### Notes on already-published deposits

The published winter-wheat deposits (filter variants, PHASE) are immutable
and retain their original `iso19157:<measure>` encoding and their original
(removed-in-v1.6.3) AGROVOC anchors as historical record. Only future builds
emit the v1.7.0 encoding. When the existing records are re-published in the
coordinated ESSD release, they pick up the verified AGROVOC layer and the
honest anchoring automatically.

### Migration

No call-site changes required. `verify_agrovoc_uris()` is opt-in via
`PhenoPhaseR.R` (report-only by default; disable with
`options(phenophaser.verify_agrovoc = FALSE)`). PIC is produced automatically
for BAM runs and skipped gracefully otherwise. Consumers that parsed
`schema:propertyID` expecting an `iso19157:` prefix should read the bare
token and follow `dqv:isMeasurementOf` → metric → `dqv:inDimension` for the
ISO dimension class.
...
[1.7.2]: https://doi.org/10.5281/zenodo.18743008
[1.7.1]: https://doi.org/10.5281/zenodo.18743008
[1.7.0]: https://doi.org/10.5281/zenodo.18743008
