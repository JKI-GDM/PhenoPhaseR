...
- **DWD plant ID 253** (sugar beet, "Zucker-Ruebe" in the DWD
  ...
  AGROVOC concept URI `http://aims.fao.org/aos/agrovoc/c_7499`
  (verified at agrovoc.fao.org) and Wikidata QID `Q151964` (verified).
  German label "Zuckerrübe".
...
- **RO-Crate profile claim corrected from Workflow Run Crate to
  Process Run Crate** in both publish hooks. The previous claim was
  aspirational: the crates declared conformance to
  `https://w3id.org/ro/crate/1.2/Workflow-Run-Crate` but did not
  include the required `ComputationalWorkflow` entity nor a
  `mainEntity` reference to it, so a `roc-validator --profile
  workflow-run-crate` check would have failed. PhenoPhaseR is an R
  pipeline of coordinated steps, not a workflow-engine-orchestrated
  workflow (no CWL / Nextflow / Snakemake definition), which is
  semantically a better fit for **Process Run Crate** — the WRROC
  base profile for "execution of one or more software applications
  that contribute to the same overall computation, but are not
  necessarily coordinated by a top-level workflow or script". The
  existing `CreateAction` with `instrument` pointing to the
  PhenoPhaseR software DOI already satisfies Process Run Crate's
  requirements unchanged. Concrete changes:
  - `conformsTo` on the metadata file descriptor now lists
    `https://w3id.org/ro/wfrun/process/0.5` instead of
    `…/Workflow-Run-Crate`.
  - A Profile Crate entity for Process Run Crate v0.5 is added to
    `@graph` per the WRROC convention.
  - The auto-written in-crate README and the HTML preview now name
    the correct profile.
  - The repository README's vocabulary table is updated to match.

  Upgrading to Workflow Run Crate proper remains a small, mechanical
  change if it ever becomes useful (FAIRagro's planned predefined-
  workflow platform is the most plausible trigger): retype the
  software DOI entity to include `ComputationalWorkflow`, add
  `mainEntity` to the root dataset pointing at it, and add the
  Bioschemas ComputationalWorkflow properties (`programmingLanguage`,
  `url`, `version`). No data-layer or numerical changes either way.

### A note on DWD phase IDs vs. BBCH numbers

DWD phase IDs are *cross-crop observation slots* drawn from
`PH_Beschreibung_PflanzePhase.txt`, not raw BBCH numbers from any
single crop scale. The same numeric code carries the same
observational meaning across crops: 10 is always "Bestellung Beginn"
(sowing), 12 is always "Auflaufen Beginn" (emergence), 24 is always
"Ernte" (harvest) — for cereals, oilseeds, root crops alike. Higher
codes (15 stem elongation, 18 heading, 19 milk ripeness, 21 yellow
ripeness, 22 full ripeness, etc.) similarly mean the same thing
wherever they appear. A consumer mapping PHASE COGs to BBCH directly
for cross-dataset analysis should use `PH_Beschreibung_Phase.txt` as
the translation table, not the published BBCH monographs alone.

### On the beet variants

DWD collects current-year beet observations under the generic
object-ID 250 ("Rueben ohne Sortenangabe", beet without variety
specification) and reassigns them after year-end to either 252
(*Futter-Ruebe*, fodder beet, *Beta vulgaris* var. *rapa* / Crassa
group) or 253 (*Zucker-Ruebe*, sugar beet) via the
`PH_Jahresmelder_Landwirtschaft_Kulturpflanze_Ruebe_Spezifizierung.txt`
file. The 252/253 mapping is per the authoritative DWD phase
catalogue.

Only the sugar-beet variety (253) is added in this release. Fodder
beet (252) is deliberately *not* included for two reasons: its
AGROVOC concept URI could not be verified at the time of release,
and sugar beet is substantially more important to current German
agriculture (~340,000 ha vs. declining cultivation of fodder beet).
Adding 252 later remains straightforward — verify the URI at
https://agrovoc.fao.org/browse/agrovoc/, then append an entry to
`.CROP_SPECS` in `_crop_specs.R` alongside the sugar-beet block.

### Action items before first sugar-beet publish

- **Mint concept DOIs** on Zenodo for the new filter-variant and
  PHASE deposits (two new DOIs) and record them in the per-call
  `upstream_doi` / `downstream_doi` arguments to the builders if you
  want them pre-baked into the manifest.
