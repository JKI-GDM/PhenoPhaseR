...
**Registered-route measure concepts in the generated vocabulary; propagated
uncertainty measures added to the shared registry; `fairagrodq` vendored at
0.3.0; the `fairagrodq:` namespace now resolves.**

A **PATCH** release for PhenoPhaseR. The RO-Crates this pipeline emits are
**unchanged**: no PhenoPhaseR measurement uses the added measure tokens or the
added `basis` label, so re-running against unchanged data reproduces
byte-identical crate metadata. The added terms belong to the standalone
`fairagrodq` vocabulary artefact, which advances **0.1.0 → 0.3.0** on its own
version line.

The scoping is deliberate and is recorded here rather than left implicit: read
against *the crates*, nothing changed, so this is a patch. Read against *the
generated vocabulary* — which does gain terms and a new node class — the same
content would be a MINOR release under the policy above. The crate-scoped
reading is taken because the crates are the external contract consumers resolve
against; the vocabulary carries its own `owl:versionInfo`. See
[`RELEASE_NOTES_v1.9.1.md`](RELEASE_NOTES_v1.9.1.md).

### Added

- **Owned measure concepts in the generated vocabulary**
  (`tools/generate_fairagrodq_ttl.R`, new step 5b). The generator now emits every
  registry measure whose `dimension` is `fairagrodq:`-owned as a
  `dqv:Metric`/`skos:Concept` node carrying `skos:prefLabel`, `skos:definition`,
  `dqv:inDimension`, and — where the measure's `basis` maps onto one —
  `fairagrodq:expressedBy`. This completes the registered route: previously the
  vocabulary published dimensions and expression principles but not the measures
  that use them, so a consumer resolving a measure IRI emitted in a crate found
  nothing. The mapping is `propagated` →
  `fairagrodq:AnalyticalErrorPropagation`, `cross-validation` / `validation` →
  `fairagrodq:EmpiricalCoverageCalibration`. As with the existing emitters, the
  nodes are derived from the registries rather than hand-written, so they cannot
  drift from what the crates emit.
- **Three propagated-uncertainty measures** in the shared registry (`u_pheno`,
  `u_precip`, `u_total`, in both `dq_vocab_core.R` and `dq_measures_registry.R`),
  each `dct:isDefinedBy` WeatherIndicatoR
  ([10.5281/zenodo.19631197](https://doi.org/10.5281/zenodo.19631197)). They
  describe the phenological-window, precipitation, and quadrature-combined
  1σ contributions to a weather-index uncertainty. PhenoPhaseR does not compute
  them; they are registered here because the vocabulary is generated from this
  registry and must cover every measure the FAIRagro measure IRIs are used for.
- **`propagated` basis label**, rendered as "analytical error propagation
  (model-asserted, unvalidated)". The parenthetical is load-bearing: a propagated
  σ is a model assertion, not an empirical coverage check, and the label says so
  at the point of use rather than leaving the distinction to the expression-
  principle axis alone.
- **`fairagrodq.ttl` vendored at the repository root** (v0.3.0, generated
  2026-08-20). The vocabulary was previously only a build artefact; shipping the
  generated file makes the terms a released crate consumer can read without
  running R, and makes vocabulary drift visible in a diff. The header retains the
  "GENERATED FILE, do not hand-edit" banner and names its source of truth.
- **`function/plot_accuracies_per-crop.R`** and
  **`function/plot_winterwheat_accuracy_uncertainty.R`** — standalone figure
  scripts for cross-crop accuracy comparison and for the winter-wheat
  accuracy/uncertainty relationship. Not wired into `PhenoPhaseR.R`; they read
  published VAM/CAL/GEM tables and are run on demand.

### Changed

- **The `fairagrodq:` namespace now resolves.** `https://w3id.org/fairagro/dq#`
  is registered; the README no longer describes registration as pending. The
  namespace remains **provisional** in the sense that matters — the terms are
  project-defined and are not part of ISO 19157-1 — and every term continues to
  declare `dct:isDefinedBy` Säurich et al. (2026) rather than asserting
  conformance to a standard that does not contain it.
- `PhenoPhaseR.R` header: the run configuration block is relabelled from
  "this run" to "example", and the per-crop Zenodo concept DOIs are removed from
  the Hook A / Hook B section comments. Both changes reflect that the driver is
  crop-generic; the concept DOIs for winter wheat remain in the run summary,
  where they are correctly scoped to plant 202.
- `.zenodo.json` — deposit metadata was last updated for v1.8.1 and described
  v1.8.0. It now carries the current version, a description covering the 1.9.x
  line, and a `references` entry for WeatherIndicatoR.

### Notes

- `tools/generate_fairagrodq_ttl.R` defines `%||%` unconditionally, while
  `dq_vocab_core.R` and `dq_uncertainty_principles.R` guard theirs with
  `if (!exists("%||%"))`. The four definitions are semantically identical, so
  behaviour is unchanged — but see `RELEASE_NOTES_v1.9.1.md` for why the
  *guarded* form is the one worth revisiting.
