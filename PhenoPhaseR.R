...
# elements. Hook B aggregates the per-(phase, year) intermediate outputs
# of spatial_interpolation.R into per-phase multi-band COGs (Cloud-
# Optimised GeoTIFFs with band names = years 1993–2024) plus one wide-
# format VAM CSV per phase. The per-year intermediate files are moved
# into a working subdirectory `_per_year/` which is excluded from the
# published ZIP. Hook B additionally embeds Data-Fitness-for-Purpose
# (DFFP) reviews if dffp_dir is supplied. Mirrors the in-pipeline
# publishing pattern of WeatherIndicatoR (Möller 2026,
# doi:10.5281/zenodo.19631197).
#
...
# processing workflow for DWD observations (v1.2.0). Zenodo.
...
plant         <- 202
target_phases <- c(10, 12, 15, 18, 19, 21, 24)            # Winter wheat
...
source(file.path(function_dir, "download_dwd_phenology5.R"))
...
source(file.path(function_dir, "plot_phenology_raster_maps (6).R"))
source(file.path(function_dir, "plot_phenology_window_timeseries.R"))
map_res <- plot_phenology_raster_maps(
  doy_file = "~/PhenoPhaseR/output/ro_crate_phase/_per_year/cogs/DOY_202-15_2018.tif",
  bse_file = "~/PhenoPhaseR/output/ro_crate_phase/_per_year/cogs/BSE_202-15_2018.tif",
  plant = 202, target_phase = 15, year = 2018,
  output_dir = "./output", aoi = "Uckermark"
)

ts_res <- plot_phenology_window_timeseries(
  pheno_dir   = "~/PhenoPhaseR/output/ro_crate_phase/_per_year/cogs",
  aoi         = "Uckermark",
...
message(
  ...
  "Hook B (PHASE COGs)      → ",
  file.path(output_dir, "ro_crate_phase.zip"), "\n",
  "\n",
  "Both ZIPs are ready for upload to Zenodo as new versions of:\n",
