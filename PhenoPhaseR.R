...
# band COGs (Cloud-Optimised GeoTIFFs with band names) plus three CSV families 
# per phase: VAM (wide cross-validation), CAL (wide BAM in-sample diagnostics: 
# AIC, BIC, EDF, deviance explained), and GEM (long-format spatial quantiles of
# the BSE uncertainty raster).
...
#
# USE OF AI TOOLS
# Parts of this code were developed with assistance from generative AI systems 
# (Anthropic Claude). All AI-assisted content was reviewed, tested, and where 
# necessary modified by the author, who takes full responsibility for the final 
# code and its behaviour.
#
# DATA SOURCE:
# Input:
...
#plant <- 203
#target_phases <- c(5, 6, 10, 12, 15, 18, 21, 24)   # Winter rye
#plant <- 204
#target_phases <- c(10, 12, 15, 18, 21, 24)         # Winter barley
#plant <- 205
#target_phases <- c(5, 10, 12, 14, 17, 22, 24, 67)  # Winter rapeseed
#plant <- 207
#target_phases <- c(10, 12, 15, 18, 21, 24)         # Spring barley
#plant <- 208
#target_phases <- c(10, 12, 15, 19, 21, 24, 66)     # Oats
#plant <- 215
#target_phases <- c(5, 10, 12, 19, 20, 21, 24, 65, 67) # Maize
#plant <- 253
#target_phases <- c(10, 12, 13, 24)                 # Sugar beet
...
# Source the shared crop-spec helper so a human-readable crop name can be 
# rendered in the run summary at the bottom of this script. Both publish
...
# Verify_agrovoc_uris() resolves every URI in _crop_specs.R against 
# agrovoc.fao.org and reports any that no longer match the concept
...
# ==============================================================================
# HOOK C - ZENODO DEPOSITION METADATA
# ==============================================================================
# Writes a complete .zenodo.json for each crate (filter variants + PHASE) into
# its crate folder, derived from the RO-Crate just written plus _crop_specs.R
# (sourced above). The AGROVOC subjects are carried identically to the crate;
# the base DOIs are read from the crate, so the metadata always matches it.
# Metadata only - push to Zenodo with tools/push_zenodo_metadata.R. Targets the
# FIRST publication (v1.0.0); for later versions see build_zenodo_metadata.R.
source(file.path(function_dir, "build_zenodo_metadata.R"))
write_zenodo_metadata(output_dir = output_dir, plant = plant)


# ==============================================================================
# Optional visualization
# ==============================================================================
# Runs after the Hook B publish step because they are excluded from the published flat layout).
