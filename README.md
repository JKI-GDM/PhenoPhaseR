...
`fairagrodq:` = `https://w3id.org/fairagro/dq#` **resolves** (w3id registration completed, v1.9.1). It remains **provisional** in the sense that matters: the terms are project-defined, are not part of ISO 19157-1, and say so. The vocabulary is generated from these same registries by [`tools/generate_fairagrodq_ttl.R`](tools/generate_fairagrodq_ttl.R) (since v1.8.1) so it cannot drift from what the crates emit, is vendored at the repository root as [`fairagrodq.ttl`](fairagrodq.ttl) (since v1.9.1), and is also published as a standalone deposit. Since v1.9.1 it carries not only the dimensions and expression principles but the **measure concepts** themselves (`dqv:Metric`), so a measure IRI emitted in a crate resolves to its definition. `dffp:` (the DFFP Application Matrix namespace) is retained **only** for the fitness-for-purpose review (`schema:potentialAction` / `AssessAction`).
...
```
PhenoPhaseR/
├── PhenoPhaseR.R                      # driver: orchestrates Steps 1-7 + both publish hooks
├── function/
│   ├── _crop_specs.R                  # per-crop registry (DWD Plant ID, AGROVOC, Wikidata, roles)
│   ├── download_dwd_phenology.R       # Step 1: DWD CDC phenology download
│   ├── couple_phenology_stations.R    # Step 2: observations x station geometry
│   ├── load_gridded_temperature.R     # Step 3: gridded daily mean temperature, keyed by GRID_ID
│   ├── effective_temperature_calculation.R  # Step 4: cumulative thermal time
│   ├── critical_doy_determination.R   # Step 5: critical temperature sums per filter strength
│   ├── filter_variant_selector.R      # Step 6: OPT scoring, filter-variant selection
│   ├── spatial_interpolation.R        # Step 7: BAM interpolation + BSE uncertainty
│   ├── dq_vocab_core.R                # registry-driven DQ engine: measures, dimensions, inDimension
│   ├── dq_measures_registry.R         # measure registry: basis, caveat, unit, definition
...
│   ├── build_zenodo_metadata.R        # deposit metadata from the crate
│   └── plot_*.R                       # figure scripts (raster maps, window time series,
│                                      #   per-crop accuracy, accuracy-vs-uncertainty)
├── tools/
│   ├── generate_fairagrodq_ttl.R      # regenerate the fairagrodq vocabulary from the registries
│   ├── push_zenodo_metadata.R         # update deposit metadata in place
│   ├── run_zenodo_deposit.R           # create / publish a deposit version
│   └── check_draft.R                  # pre-publication draft checks
├── fairagrodq.ttl                     # generated vocabulary, vendored (do not hand-edit)
├── CHANGELOG.md
├── CITATION.cff
├── LICENSE
├── .zenodo.json
...
The `plot_*.R` scripts are not called by the driver except where noted in `PhenoPhaseR.R`; they read published VAM/CAL/GEM tables and are run on demand.
...
source("function/dq_vocab_core.R")
source("function/dq_uncertainty_principles.R")
source("function/build_filtervariant_ro_crate.R")
source("function/build_phase_cog_ro_crate.R")
