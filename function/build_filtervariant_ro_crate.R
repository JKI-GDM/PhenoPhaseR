...
.per_year_quality_measurements <- function(q_row, plant, phase, year) {
  ...
  qe <- add_qe(qe, "RMSE",           "is_rmse",                "days",
               "Root mean squared error between predicted and observed DOY")
  qe <- add_qe(qe, "MAE",            "is_mae",               "days",
...
build_filtervariant_ro_crate <- function(
  ...
  # iso19157:QuantitativeAttributeAccuracy, 2023 ed.). Many measurements share the same Metric;
  ...
  # ISO 19157-1 elements + provisional fairagrodq: dimensions as @graph nodes. Only
  ...
  crate <- list(
    "@context" = list(
      ...
      list(
        ...
        # Owned, PROVISIONAL namespace for project-defined spatial-uncertainty
        # dimensions; now consistent with the PHASE builder via dq_vocab_core.R.
        "fairagrodq" = "https://w3id.org/fairagro/dq#",
        # ISO 19157-1:2023 quality elements (the provisional fairagrodq:
        # extension dimensions are declared above), referenced via
        # dqv:inDimension on the dqv:Metric entities and
        ...
        # DFFP review-tool vocabulary only (NOT a quality-dimension namespace);
        # retained for cross-builder parity. Spatial-uncertainty dimensions are
        # under fairagrodq: above, consistent with the PHASE builder.
