...
.write_crate_readme <- function(out_dir, plant, phase, years,
  ...
  txt <- c(
    ...
    paste("- The [NovaCrate](https://novacrate.datamanager.kit.edu/) editor",
          ...
          "RO-Crate violations."),
    paste("- Properties under the custom `dffp:` namespace",
          "(`https://github.com/MahdiHedayatMahmoudi/dffp-application-matrix`)",
          "are flagged as unresolved by validators that fetch schemas at",
          "validation time, because that namespace does not currently",
          "dereference to a machine-readable RDF schema."),
    "",
