...
build_phase_cog_ro_crate <- function(
  ...
  publishable <- setdiff(
    list.files(out_dir, recursive = TRUE, all.files = FALSE,
               full.names = FALSE, include.dirs = FALSE),
    list.files(file.path(out_dir, "_per_year"), recursive = TRUE,
               all.files = FALSE, full.names = FALSE, include.dirs = FALSE)
  )
