...
.write_crate_readme <- function(out_dir, plant, phase, years,
  ...
  txt <- c(
    ...
    sprintf("%d\u2013%d axis (32 bands), so band index always maps to year: ", min(years), max(years)),
    "band *i* corresponds to year ",
    sprintf("%d + *i* \u2212 1. ", min(years)),
    "Where no interpolated surface could be produced for a given year ",
    "(either because the crop-phase was not reported by DWD that year, or ",
    "because too few stations passed filtering to interpolate a stable ",
    "surface), that year is present as a **full-extent all-NA band** rather ",
    "than being omitted. This keeps the band-to-year mapping stable across ",
    "all phases and lets `terra::rast(file, lyrs = \"<year>\")` return a clean ",
    "NA layer instead of throwing for a missing year.",
    "",
    "The specific years affected, and the reason for each gap, are recorded ",
    "in `ro-crate-metadata.json` as `dqv:QualityMeasurement` entities ",
    "(ISO 19157-1 `DQ_CompletenessOmission`) attached to the relevant COG, ",
    "and in each NA band's GDAL band description (visible via ",
    "`gdalinfo cogs/DOY_<plant>-<phase>.tif`). To detect NA years ",
    "programmatically:",
    "",
    "```r",
    "library(terra)",
    "r <- terra::rast(\"cogs/DOY_202-15.tif\")",
    "# Which year-bands are entirely NA?",
  ...
  if (length(gap_log)) {
    ...
    for (k in unique(key)) {
      ...
      gap_entities[[length(gap_entities) + 1L]] <- list(
        "@id"   = gid,
        "@type" = c("schema:PropertyValue", "dqv:QualityMeasurement"),
        "schema:propertyID" = "iso19157:DQ_CompletenessOmission",
