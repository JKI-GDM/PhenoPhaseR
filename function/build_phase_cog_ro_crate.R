...
# Domain-neutral, registry-driven DQ vocabulary engine. Supersedes the local
# .iso19157_dimension/.metric_entity/.quality_element defined further below;
# .dimension_entities() and .with_bse_uncertainty_anchors() now live here.
source(file.path(.this_dir, "dq_vocab_core.R"))
# Orthogonal spatial-uncertainty expression-principle axis (fairagrodq:expressedBy).
...
# Data-quality vocabulary engine (.iso19157_dimension / .quality_element /
# .metric_entity) now lives in dq_vocab_core.R, sourced above. The registry
# there reproduces the previous switch exactly; back-compat aliases keep these
# names working at the call sites below.
...
.layer_description <- function(layer, plant, phase, years) {
  ...
  switch(layer,
    ...
    BSE = sprintf(paste("Basis-spline standard error of the BAM posterior for",
                  ...
                  plant, phase, n_yr, yr_min, yr_max),
    VAM = sprintf(paste("Hold-out accuracy metrics for plant %d,",
    ...
    PIC = sprintf(paste("Prediction-interval coverage (PIC) — calibration of",
                        "the per-pixel BSE uncertainty for plant %d, phase %d.",
                        "Reports the prediction-interval coverage probability",
                        "(PICP) and mean prediction-interval width (MPIW) from",
                        "a dedicated k-fold cross-validation — i.e. whether",
                        "the BSE-derived intervals are well calibrated. Wide-",
                        "format CSV with one row per year (%d–%d, %d rows)."),
                  plant, phase, yr_min, yr_max, n_yr)
  )
}
...
.phase_dataset <- function(phase, art, crate_root, plant, years, q_phase) {
  ...
  # ---- BSE spatial-uncertainty distribution lifted from GEM ---------------
  # The per-pixel BSE raster's spatial spread (GEM quantiles) ships as a CSV
  # File part; also surface it as measurements so the spatial VARIATION of
  # uncertainty is queryable. computedOn the BSE COG; dimension
  # fairagrodq:NumericalSpatialUncertainty and principle
  # fairagrodq:ParametricModelUncertainty are assigned by the engine fallbacks.
  if (length(gem_path) && file.exists(gem_path) && length(bse_path)) {
    gem_tab <- tryCatch(utils::read.csv2(gem_path, stringsAsFactors = FALSE),
                        error = function(e) NULL)
    if (!is.null(gem_tab) && nrow(gem_tab)) {
      gem_qe <- dq_spatial_quantile_measurements(
        gem_tab, cog_relpath = .relpath(bse_path, crate_root),
        token_prefix = "bse_q", id_stub = sprintf("%d-%d", plant, phase),
        unit_text = "days")
      for (e in gem_qe) qe[[length(qe) + 1]] <- e
    }
  }
  ...
  dataset <- list(
    ...
    "description" = paste0(
      ...
      "format VAM CSV with one row per year (hold-out validation), ",
...
.write_crate_readme <- function(out_dir, plant, phase, years,
  ...
  txt <- c(
    ...
    "- `vam/VAM_<plant>-<phase>.csv`  — hold-out validation metrics, one row per year.",
    ...
    "",
    paste("Note on the `fairagrodq:` namespace: per-pixel uncertainty has no",
          "element in ISO 19157-1:2023. This crate types it under a project-",
          "defined, PROVISIONAL dimension",
          "(`fairagrodq:NumericalSpatialUncertainty`) in an owned namespace",
          "(`https://w3id.org/fairagro/dq#`, pending registration), informed",
          "by Saeurich et al. (2026, doi:10.1016/j.ecoinf.2026.103660). It is",
          "NOT claimed as part of ISO 19157-1. The quantity's subject is",
          "anchored in AGROVOC (`c_28975` statistical uncertainty +",
          "`c_379bbe9f` spatial data)."),
    "",
...
build_phase_cog_ro_crate <- function(
  ...
  # Annotate each metric with its spatial-uncertainty expression PRINCIPLE
  # (fairagrodq:expressedBy). Accuracy/evidence metrics get none (honest).
  ...
  for (layer in c("DOY", "BSE")) {
    for (p in art[[layer]]) {
      ...
      # BSE COG carries the spatial-uncertainty anchors: subjects AGROVOC
      # c_28975 + c_379bbe9f; conformsTo fairagrodq:NumericalSpatialUncertainty
      # (provisional). DOY COG is left as-is.
      if (identical(layer, "BSE")) {
        fe <- .with_bse_uncertainty_anchors(fe)
        # PRODUCTION principle of the BSE raster: model-internal posterior SE.
        fe <- dq_attach_principle(fe, "fairagrodq:ParametricModelUncertainty")
      }
      file_entities <- c(file_entities, list(fe))
    }
  }
  for (csv_layer in c("VAM", "CAL", "GEM", "PIC")) {
  ...
  all_outputs <- unname(unlist(c(art$DOY, art$BSE, art$VAM, art$CAL, art$GEM, art$PIC)))
  create_action <- list(
  ...
  root_dataset <- list(
    ...
    "schema:potentialAction" = list("@id" = dffp_action[["@id"]]),
  ...
  # ISO 19157-1 dimensions + provisional fairagrodq: dimensions as @graph nodes. Only
  # the dimensions actually referenced by a metric in THIS crate (plus their
  # skos:broader parents) are emitted, so each crate defines exactly what it
  # uses and every dqv:inDimension target resolves.
  used_dims <- unique(unlist(lapply(metric_entities,
                            function(m) m[["dqv:inDimension"]][["@id"]])))
  if (is.null(used_dims)) used_dims <- character(0)
  context_entities <- c(context_entities, dq_dimension_entities(used_dims))
  # Spatial-uncertainty expression-principle nodes (+ scheme + predicate defs)
  # for the principles actually referenced by a metric or an uncertainty layer.
  used_pr <- dq_principles_used(metric_entities, extra_files = file_entities)
  ...
  crate <- list(
    "@context" = list(
      ...
      list(
        ...
        # RDF/RDFS for the owned uncertainty-principle predicate definitions
        # (rdf:Property; rdfs:label/comment/seeAlso) in dq_uncertainty_principles.R.
        "rdf"      = "http://www.w3.org/1999/02/22-rdf-syntax-ns#",
        "rdfs"     = "http://www.w3.org/2000/01/rdf-schema#",
        # ISO 19157-1:2023 quality elements + provisional fairagrodq: dimensions,
        # referenced via dqv:inDimension on the dqv:Metric entities and
        # defined as @graph nodes by .dimension_entities(). No
        # iso19157:<measure> IRIs are minted; 2023 omits the "DQ_" prefix.
        ...
        # Owned, PROVISIONAL namespace for project-defined quality dimensions
        # (spatial uncertainty); pending FAIRagro/w3id registration.
        "fairagrodq" = "https://w3id.org/fairagro/dq#",
        # DFFP review-tool vocabulary -- fitness-for-purpose review CATEGORIES
        # only (schema:propertyID on the review PropertyValues); documented in
        # the DFFP Application Matrix deposit, Zenodo 19693642. NOT the home of
