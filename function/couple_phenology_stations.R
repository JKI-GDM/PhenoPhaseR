...
couple_phenology_stations <- function(
  ...
  # Import shapefile using sf (modern standard, replaces deprecated raster::shapefile)
  # This preserves spatial reference system (CRS)
  stations <- sf::st_read(paste0(station_file, ".shp"), quiet = TRUE)
  # Convert to sp for downstream sp::merge() compatibility
  ...
  if (is.element(plant, winter_crops) &
    ...
    # NOTE: Must use sp::merge() here, NOT base merge(), to preserve the
    # SpatialPointsDataFrame class. Base merge() silently strips geometry.
    stations <- sp::merge(
      stations,
  ...
  sf::st_write(sf::st_as_sf(stations), output_path, delete_layer = TRUE)
  return(stations)
}
