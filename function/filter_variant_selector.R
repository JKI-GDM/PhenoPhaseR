...
filter_variant_selector <- function(
  ...
  # Gap log: one row per (phase, year) that is intentionally dropped because
  # no filter variant met the min_obs / min_cor constraints. Written as
  # GAPS_<plant>.csv so the downstream interpolation and the Hook B publisher
  # know WHY a given (phase, year) has no optimal shapefile, rather than
  # treating the absence as a fatal error. This is the authoritative
  # detection point for the "insufficient samples" case because min_obs is
  # evaluated here against SN (the per-variant sample count).
  #
  # The log lives in an environment and is flushed via on.exit so that even
  # if a later phase errors (e.g. in diagnostics), the gaps detected so far
  # are still written. The normal end-of-function write (below) is the
  # primary path; this is the safety net.
  gap_env <- new.env(parent = emptyenv())
  gap_env$rows <- list()
  gap_path <- file.path(out_csv, paste0("GAPS_", plant, ".csv"))
  .flush_gaps <- function() {
    if (!length(gap_env$rows)) return(invisible(NULL))
    gd <- do.call(rbind, gap_env$rows)
    gd <- gd[order(gd$PHASE, gd$YEAR), , drop = FALSE]
    tryCatch(utils::write.csv2(gd, file = gap_path, row.names = FALSE),
             error = function(e) NULL)
  }
  on.exit(.flush_gaps(), add = TRUE)
  ...
  for (phase in phases) {
    ...
    for (yr in years) {
      ...
      if (length(valid_opts) == 0) {
        ...
        # Record WHY this (phase, year) is dropped so the absence of an
        # optimal shapefile downstream is documented, not fatal. Inspect the
        # un-filtered rows for this year to attribute the cause precisely.
        yr_rows <- df_cv[df_cv$YEAR == yr, , drop = FALSE]
        max_sn  <- if (nrow(yr_rows)) suppressWarnings(max(yr_rows$SN,  na.rm = TRUE)) else NA_real_
        max_cor <- if (nrow(yr_rows)) suppressWarnings(max(yr_rows$COR, na.rm = TRUE)) else NA_real_
        if (!is.finite(max_sn))  max_sn  <- NA_real_
        if (!is.finite(max_cor)) max_cor <- NA_real_
        reason <- if (!is.na(max_sn) && max_sn < min_obs) {
          sprintf("insufficient_samples (max SN=%g < min_obs=%g)", max_sn, min_obs)
        } else if (!is.na(max_cor) && max_cor < min_cor) {
          sprintf("low_correlation (max COR=%.3f < min_cor=%.3f)", max_cor, min_cor)
        } else {
          "no_valid_variant (min_obs/min_cor constraints not met)"
        }
        gap_env$rows[[length(gap_env$rows) + 1L]] <- data.frame(
          PLANT  = plant, PHASE = phase, YEAR = yr,
          MAX_SN = max_sn, MAX_COR = max_cor,
  ...
  if (length(all_results) == 0) {
    ...
    # Still write the gap log so downstream code has a record of why every
    # phase/year was dropped, instead of silently finding no shapefiles.
    if (length(gap_env$rows)) {
      gap_df <- do.call(rbind, gap_env$rows)
      gap_df <- gap_df[order(gap_df$PHASE, gap_df$YEAR), , drop = FALSE]
      utils::write.csv2(gap_df, file = gap_path, row.names = FALSE)
      if (verbose)
        cat("Wrote gap log: ", basename(gap_path), " (",
            nrow(gap_df), " dropped phase/year combination(s))\n", sep = "")
    }
  ...
  # Write the gap log (dropped phase/year combinations). Always write the
  # file — an empty file with just a header when there are no gaps — so the
  # downstream interpolation and publisher can rely on its presence.
  if (length(gap_env$rows)) {
    gap_df <- do.call(rbind, gap_env$rows)
    gap_df <- gap_df[order(gap_df$PHASE, gap_df$YEAR), , drop = FALSE]
  } else {
    gap_df <- data.frame(PLANT = integer(0), PHASE = integer(0),
                         YEAR = integer(0), MAX_SN = numeric(0),
                         MAX_COR = numeric(0), REASON = character(0),
  ...
  if (verbose) {
    if (nrow(gap_df))
      ...
      cat("  - Wrote gap log:", basename(gap_path), "(no gaps)\n")
  }
