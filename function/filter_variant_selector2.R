...
# out_dir - Character. Directory containing output files. 
...
#                     (default 0.5).
#
# min_obs           - Numeric. Minimum acceptable number of observations (SN)
#                     to consider a variant viable (default 40). Prevents
#                     selection of variants that would result in unstable 
...
filter_variant_selector <- function(
  ...
  for (phase in phases) {
    ...
    # Apply minimum observation constraint
    ...
    utils::write.csv2(
      df_cv,
      file = paste0(out_dir,out_all_name),
      row.names = FALSE
    )
    ...
    for (yr in years) {
      ...
      if (length(valid_opts) == 0) {
        warning("No valid OPT values for year ", yr, ", phase ", phase, " (min_cor or min_obs constraints not met)")
        next
      }
      ...
      if (requireNamespace("sf", quietly = TRUE)) {
        shp_file <- paste0("DOY_", df_year_best$PLANT, "-", df_year_best$PHASE,
        ...
        if (file.exists(shp_file)) {
          ...
          sf::st_write(opt_shp, paste0(out_dir,out_shp), delete_layer = TRUE, quiet = TRUE)
        }
      }
    }
    ...
    utils::write.csv2(
      df_cv_max,
      file = paste0(out_dir,out_max_name),
      row.names = FALSE
    )
  ...
  utils::write.csv2(
    df_all_combined,
    file = paste0(out_dir,"OPT_ALL_", plant, "_ALL_PHASES.csv"),
    row.names = FALSE
  )
  
  utils::write.csv2(
    df_best_combined,
    file = paste0(out_dir,"OPT_MAX_", plant, "_ALL_PHASES.csv"),
    row.names = FALSE
  )
  
  utils::write.csv2(
    df_params_combined,
    file = paste0(out_dir,"OPT_", plant, "_EXPONENTS_ALL_PHASES.csv"),
    row.names = FALSE
  )
...
export_diagnostic_plots <- function(df_all, df_best, year_params, 
  ...
  grDevices::pdf(paste0(out_dir,pdf_name), width = 11, height = 8.5)
...
# )
# 
## Custom exponent range & minimum observations
# results <- filter_variant_selector(
#   in_dir = "./output/",
#   plant = 202,
#   phases = 24,
