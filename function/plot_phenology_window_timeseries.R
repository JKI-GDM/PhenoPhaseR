...
plot_phenology_window_timeseries <- function(
  ...
  #   Analysis CRS is EPSG:25832 (PHASE / PhenoPhaseR native), which avoids
  ...
  message("Loading PHASE COGs and aggregating to AOI ...")
  ...
  p <- p +
    ...
    labs(
      ...
      y       = "Day of year (DOY)",
      caption = paste0(
        "Layout after Krengel-Horney et al. (2021), JfK 73(7-8): 292-305. ",
        "Uncertainty: BAM posterior standard error (BSE) from PHASE / ",
        "PhenoPhaseR (M\u00f6ller et al. 2019, 2026)."
      )
    ) +
    essd_theme
